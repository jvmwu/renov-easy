...
mod tests {
    ...
    fn test_auth_response_new() {
        ...
        assert_eq!(response.user_type, Some("customer".to_string()));
        assert!(!response.requires_type_selection);
    }

    #[test]
    fn test_auth_response_from_token_pair_with_user_type() {
        ...
        assert_eq!(response.access_token, "access_token");
        assert_eq!(response.refresh_token, "refresh_token");
        assert_eq!(response.expires_in, 900); // 15 minutes
        assert_eq!(response.user_type, Some("worker".to_string()));
        assert!(!response.requires_type_selection);
    }

    #[test]
    fn test_auth_response_from_token_pair_without_user_type() {
        let token_pair = TokenPair::new(
            "access_token".to_string(),
            "refresh_token".to_string(),
        );

        let response = AuthResponse::from_token_pair(token_pair, None);
